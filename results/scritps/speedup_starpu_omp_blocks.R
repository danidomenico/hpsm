...
dados <- dados[( ((dados$BENCH == "hotspot") & (dados$size == 16384)) | 
                 ...
                 ((dados$backend == "PARALLEL_BACK_STARPU") | (dados$backend == "PARALLEL_BACK_STARPU_OPENMP")),]
...
dados$backend <- ifelse(dados$backend == "PARALLEL_BACK_STARPU" & 
                        ((dados$BENCH == "hotspot" & dados$block_size == 1024) |
                         ...
                         (dados$BENCH == "cfd"     & dados$block_size == 4096)), "StarPU-small", as.character(dados$backend))
dados$backend <- ifelse(dados$backend == "PARALLEL_BACK_STARPU" & 
                        ((dados$BENCH == "hotspot" & dados$block_size == 2048) |
                         (dados$BENCH == "nbody"   & dados$block_size == 4096) |
                         (dados$BENCH == "cfd"     & dados$block_size == 8192)), "StarPU-big", as.character(dados$backend))
dados$backend <- ifelse(dados$backend == "PARALLEL_BACK_STARPU_OPENMP" &
                        ((dados$BENCH == "hotspot" & dados$block_size == 1024) |
                         (dados$BENCH == "nbody"   & dados$block_size == 2048) |
                         (dados$BENCH == "cfd"     & dados$block_size == 4096)), "StPU+OMP-small", as.character(dados$backend))
dados$backend <- ifelse(dados$backend == "PARALLEL_BACK_STARPU_OPENMP" & 
                        ((dados$BENCH == "hotspot" & dados$block_size == 2048) |
                         (dados$BENCH == "nbody"   & dados$block_size == 4096) |
                         (dados$BENCH == "cfd"     & dados$block_size == 8192)), "StPU+OMP-big", as.character(dados$backend))
