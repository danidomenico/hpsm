...
dados <- dados[((dados$BENCH == "hotspot") & (dados$size == 16384) & (dados$block_size == 1024)) | 
					...
					((dados$BENCH == "cfd") & (dados$size == 131072) & (dados$block_size == 4096)),]
...
dados$backend <- ifelse(dados$backend == "PARALLEL_BACK_STARPU_OPENMP", "StPU+OMP", as.character(dados$backend))
